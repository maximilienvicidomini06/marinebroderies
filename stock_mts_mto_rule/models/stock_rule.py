...
class StockRule(models.Model):
    ...
    def _run_split_procurement(self, procurements):
        ...
        for procurement_rule in procurements:
            procurement, _rule = procurement_rule
            needed_qty = _rule.get_mto_qty_to_order(
            ...
            if float_is_zero(needed_qty, precision_digits=precision):
                getattr(self.env["stock.rule"], "_run_%s" % _rule.mts_rule_id.action)(
                    [(procurement, _rule)]
                )
            elif (
                float_compare(
                ...
                getattr(self.env["stock.rule"], "_run_%s" % _rule.mto_rule_id.action)(
                    [(procurement, _rule)]
                )
            else:
                mts_qty = procurement.product_qty - needed_qty
                mts_procurement = procurement._replace(product_qty=mts_qty)
                proc = (mts_procurement, _rule.mts_rule_id)
                getattr(self.env["stock.rule"], "_run_%s" % _rule.mts_rule_id.action)(
                    [proc]
                )
                ...
                getattr(self.env["stock.rule"], "_run_%s" % _rule.mto_rule_id.action)(
                    [(mto_procurement, _rule.mto_rule_id)]
                )
        return True
